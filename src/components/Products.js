...
const Products = () => {
    ...
    if(token){
      return(
        <Box className="cartBox">
          <Cart products={productList} items={items}  handleQuantity={(items,products,productId,qty) => addToCart(token,items,products,productId,qty)}/>
        </Box>
      )
    }
  }
  ...
  const fetchCart = async (token) => {
    ...
    try {
      ...
      if (e.response && e.response.status === 400) {
        ...
        enqueueSnackbar(
          "Could not fetch cart details. Check that the backend is running, reachable and returns valid JSON. Thisisloggingit",
          {
            variant: "error",
          }
        );
  ...
  const addToCart = async (
    ...
    try {
      ...
      const cartDetails = await generateCartItemsFrom(response.data,products)
      ...
      if (e.response && e.response.status === 400) {
        ...
        enqueueSnackbar(
          "Could not fetch cart details. Check that the backend is running, reachable and returns valid JSON. NotThis",
          {
            variant: "error",
          }
        );
                    ...
                    productList.map((item) => {
                      return(
                        <Grid item xs={6} md={3}  key={item._id}>
                          <ProductCard product={item} handleAddToCart={(id) => addToCart(token,items,productList,id,1,{preventDuplicate:true})}/>
                        </Grid>
                      )
                    })
                  ) : (
